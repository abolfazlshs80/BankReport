...
namespace BankReport.Services
{
    ...
    public class KeshavarziMessageParser : IMessageParser
    {
        public bool CanParse(string message, string phoneNumber)
        {
            // Identify based on sender name or known numbers
            return phoneNumber.Contains("Bank Keshavarzi") || phoneNumber.Contains("5");
        }
...
pattern);
            ...
            if (debitMatch.Success)
            {
                ...
                transaction.AccountId = debitMatch.Groups["card"].Value;
...
^(?<type>برداشت|واریز|واريز)[:：]?
(?<sign_before>[-+])?
\s*(?<amount>[0-9۰-۹٬,]+)
\s*(?<sign_after>[-+])?
\s*\nمانده[:：]?\s*(?<balance>[0-9۰-۹٬,]+)
\s*\n(?<date>\d{6})-(?<time>\d{1,2}:\d{2})
\s*\nکارت\s*(?<card>[0-9۰-۹\*]+)";
    }
