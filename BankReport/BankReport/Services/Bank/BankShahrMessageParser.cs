...
namespace BankReport.Services
{
    ...
    public class BankShahrMessageParser : IMessageParser
    {
        public bool CanParse(string message, string phoneNumber)
        {
            // Identify based on sender name or known numbers
            return phoneNumber.Contains("Bank Shahr") || phoneNumber.Contains("1512127274495");
        }
...
^\*بانک\s+شهر\*\s*
انتقال\s+وجه\s+کارتي\s*
(?<type>برداشت\s+از|واريز\s+به)[:：]?\s*(?<account>[0-9۰-۹]+)\s*
مبلغ[:：]?\s*(?<amount>[0-9۰-۹٬,]+)ريال\s*
موجودي[:：]?\s*(?<balance>[0-9۰-۹٬,]+)\s*ريال\s*
(?<date>\d{4}/\d{1,2}/\d{1,2})\s+(?<time>\d{1,2}:\d{2}:\d{2})";
    }
